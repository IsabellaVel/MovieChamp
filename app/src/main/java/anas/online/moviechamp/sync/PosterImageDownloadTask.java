...
public class PosterImageDownloadTask extends AsyncTask<String, Void, String> {

    private static final String LOG_TAG = PosterImageDownloadTask.class.getSimpleName();
    private static Context context = MovieChamp.getContext();

    public PosterImageDownloadTask() {
    ...
    protected String doInBackground(String... urls) {
        ...
        String mExternalposterPath = urls[0];

        Log.v(LOG_TAG, "path = " + mExternalposterPath);

        try {

            URL url = new URL(mExternalposterPath);
            ...
            String[] parts = mExternalposterPath.split("/");
            String filename = parts[6];
            //Log.i(LOG_TAG, "filename: " + filename);

            File file = new File(PosterImageDownloadTask.context.getExternalFilesDir(
            ...
            while ((bufferLength = inputStream.read(buffer)) > 0) {
                ...
                //   Log.i(LOG_TAG, "Progress: downloaded size "  + downloadedSize + "Total size: " + totalSize)
            }
            fileOutput.close();
